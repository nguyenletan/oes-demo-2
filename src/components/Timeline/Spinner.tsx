interface ISpinnerProps {
  color?: string
  status?: string
}

export default function Spinner(props: ISpinnerProps) {
  const { color, status } = props
  if (status === 'overdue') {
    return (
      <svg className="pin" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
          d="M10 0C4.48 0 0 4.48 0 10C0 15.52 4.48 20 10 20C15.52 20 20 15.52 20 10C20 4.48 15.52 0 10 0ZM11 15H9V13H11V15ZM11 11H9V5H11V11Z"
          fill="#EB1919"
        />
      </svg>
    )
  }

  return (
    <svg className="pin" width="14" height="20" viewBox="0 0 14 20" fill={color} xmlns="http://www.w3.org/2000/svg">
      <path d="M7 0C3.13 0 0 3.13 0 7C0 12.25 7 20 7 20C7 20 14 12.25 14 7C14 3.13 10.87 0 7 0ZM7 9.5C5.62 9.5 4.5 8.38 4.5 7C4.5 5.62 5.62 4.5 7 4.5C8.38 4.5 9.5 5.62 9.5 7C9.5 8.38 8.38 9.5 7 9.5Z" />
      <path
        d="M9.5 7C9.5 8.38 8.38 9.5 7 9.5C5.62 9.5 4.5 8.38 4.5 7C4.5 5.62 5.62 4.5 7 4.5C8.38 4.5 9.5 5.62 9.5 7Z"
        fill="white"
      />
    </svg>
  )
}
